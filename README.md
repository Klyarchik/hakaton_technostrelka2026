...
4. Для того, чтобы видеть базовые аватары пользователей, надо: 
    ...
    4) скачать файл default-avatar.png  (любая дефолтная аватарка пользователя) и добавить его в этот bucket

5. При клонировании проекта и в деплое push-уведомления (если запускать локально, то необходима база firebase, но ключ от нее github при сохранении изменений не пропускает, а в деплое firebase работает только с https-запросами)
